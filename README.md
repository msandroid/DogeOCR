...
- **リアルタイム処理**: 数秒での認証完了
- **高精度顔認証**: Vison AI APIによる専門的な顔照合

### Doge eKYC（新機能）
- **8段階認証フロー**: 包括的で詳細な認証プロセス
- **QRコード連携**: デスクトップとモバイルの安全な連携
- **年齢確認機能**: 実年齢計算による成年判定
- **セキュリティ強化**: TLS暗号化・画像即座削除・認証トークン
- **データ最小化**: 年齢確認に必要な情報のみ処理
...
- 顔認証（Vison AI API - 身分証明書 vs セルフィー）
- 真贋判定
- 年齢・有効期限チェック
- 7年間の記録保存
- 顔画像品質評価
...
### eKYC認証機能（新機能）
- **8段階認証フロー**: 身分証撮影→OCR処理→セルフィー撮影→顔認証→年齢確認→セキュリティ処理→総合判定→結果表示
- **QRコード連携**: デスクトップでQRコード表示、モバイルで認証実行
- **セッション管理**: 安全なセッション管理によるリアルタイム通信
- **年齢確認**: 実年齢計算による成年判定（18歳以上）
- **セキュリティ機能**: TLS暗号化・画像即座削除・認証トークン・データ最小化
